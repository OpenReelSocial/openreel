...
| Alembic (Python) for migrations, Kysely for queries | Considered because part of the team already knows Alembic well. It would run fine against a database that only TypeScript reads, but it adds a Python toolchain to every backend checkout, container image, and CI job for one concern, and without SQLAlchemy models on the service side there is nothing for `--autogenerate` to diff, so migrations would be hand-written `op.*` calls anyway. ADR-0002 confines Python to `services/classifier/`. |
